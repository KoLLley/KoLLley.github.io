...
function adaptive_header(w,h) {
		...
		var headerLanguages=$('.header-body-top_languages');
	if(w<767){
		if(!headerLanguages.hasClass('done')){
			headerLanguages.addClass('done').appendTo(headerMenu);
		}
	}else{
		if(headerLanguages.hasClass('done')){
			headerLanguages.removeClass('done').prependTo($('.header-body-top'));
		}
	}

	if(w<767){
		if(!$('.header-body-bottom__menu').hasClass('done')){
			$('.header-body-bottom__menu').addClass('done').appendTo(headerMenu);
		}
	}else{
		$.each($('.header-body-bottom__menu'), function(index, val) {
			if($(this).hasClass('header-body-bottom__menu_right')){
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.header-body-bottom__column').eq(2));
				}
			}else{
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.header-body-bottom__column').eq(0));
				}
			}
		});
	}
